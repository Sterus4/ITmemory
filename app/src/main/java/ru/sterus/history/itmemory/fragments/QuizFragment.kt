...
import ru.sterus.history.itmemory.ItemRVOptionClicked
import ru.sterus.history.itmemory.R
import ru.sterus.history.itmemory.RVQuestionOptionsAdapter
...
class QuizFragment : Fragment(), ItemRVOptionClicked {
    ...
    private fun setupQuestionOptions(options: ArrayList<QuizQuestionOption>) {
        options.shuffle()
        val questionOptionsLayoutManager = LinearLayoutManager(context, RecyclerView.VERTICAL, false)
        val questionOptionsRVAdapter = RVQuestionOptionsAdapter(options, context, this)
    ...
    override fun onItemClicked(q0: QuizQuestionOption, parent: CardView) {
        if (q0.isCorrect){
            vb.nextQuestionButton.visibility = View.VISIBLE
            parent.setCardBackgroundColor(ContextCompat.getColor(requireContext(),
                R.color.win_green
            ))
        } else {
            parent.setCardBackgroundColor(ContextCompat.getColor(requireContext(), R.color.red_bad))
        }
        Snackbar.make(vb.root, q0.feedback, Snackbar.LENGTH_LONG).show()
    }
